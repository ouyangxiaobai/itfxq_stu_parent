...
public class FileController {
    ...
    private String newsUploadFilePath;
    //课程介绍视频上传地址
    @Value("${courseVideo.upload.path}")
    private String videoUploadFilePath;
    ...
    public ResResult<Map<String, Object>> upload(HttpServletRequest req, @RequestParam("file") MultipartFile file){
        try {
            ...
            String uploadPath = uplaodFilePath;
    ...
    @PostMapping(value="/news/file/uploadFile")
    @ResponseBody
    public ResResult<Map<String, Object>> uploadProductFmUrl(HttpServletRequest req, @RequestParam("file") MultipartFile file){
        try {
            Map<String, Object> resultMap = new HashMap();
            if(file.isEmpty()){
                resultMap.put("message","文件为空");
            ...
            String uuidString = UUID.randomUUID().toString();
            String newFileName= uuidString + suffixName;
            String uploadPath = newsUploadFilePath;
            File path = new File(uploadPath);
            if (!path.exists()) path.mkdirs();
    ...
    @RequestMapping(value = "/news/showimage")
    public String showProductImage(@RequestBody  Map mp,HttpServletRequest request, HttpServletResponse response)
            throws Exception {
    ...
    //上传视频 course/file/uploadVideo

    @PostMapping(value="/course/file/uploadVideo")
    @ResponseBody
    public ResResult<Map<String, Object>> uploadVideo(HttpServletRequest req, @RequestParam("file") MultipartFile file){
        try {
            Map<String, Object> resultMap = new HashMap();
            if(file.isEmpty()){
                resultMap.put("message","文件为空");
            ...
            String uuidString = UUID.randomUUID().toString();
            String newFileName= uuidString + suffixName;
            String uploadPath = videoUploadFilePath;
            File path = new File(uploadPath);
            if (!path.exists()) path.mkdirs();
    ...
    @RequestMapping(value = "/course/showVideo")
    public String showVideo(@RequestBody  Map mp,HttpServletRequest request, HttpServletResponse response)
            throws Exception {
        ...
        File fileParent = new File(File.separator);
        // 获得/usr/CBeann目录
        System.out.println("视频名称:"+image_name);
        File file = null ;
        String os = System.getProperty("os.name");
        ServletOutputStream out = response.getOutputStream();
        try {
